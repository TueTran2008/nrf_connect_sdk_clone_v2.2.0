...
static void pair_timer_handle(struct k_timer *timer)
{
	...
	// else
	// {
		struct event_t nus_event;
		nus_event.event_source = EVT_SRC_TIMER;
		nus_event.nus_central_activity.event = NUS_CENTRAL_EVENT_PAIR_TIMEOUT;
		if(ctrl_events_queue_empty())
		{
			ctrl_events_put(&nus_event);
		}
	//}
}
