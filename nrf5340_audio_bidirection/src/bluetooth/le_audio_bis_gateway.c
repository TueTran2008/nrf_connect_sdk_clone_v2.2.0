...
static void get_mac_address(uint8_t *mac_address) {
    // mac_address[0] = ((NRF_FICR->INFO.DEVICEID[0] & 0x0000FF00) >> 8) | 0b11000000;
    // mac_address[1] = NRF_FICR->INFO.DEVICEID[1] & 0x000000FF;
    // mac_address[2] =  ((NRF_FICR->INFO.DEVICEID[0] & 0xFF000000) >> 24);
    // mac_address[3] = ((NRF_FICR->INFO.DEVICEID[0] & 0x00FF0000) >> 16);
    // mac_address[4] = ((NRF_FICR->INFO.DEVICEID[0] & 0x0000FF00) >> 8);
    // mac_address[5] = ((NRF_FICR->INFO.DEVICEID[0] & 0x000000FF) >> 0);
}
static int adv_create(void)
{
	...
	char name[64];
	sprintf(name,"%s-%02x:%02x:%02x:%02x:%02x:%02x", CONFIG_BT_DEVICE_NAME
											 , mac_addr.a.val[0]
											 , mac_addr.a.val[1]
											 , mac_addr.a.val[2]
											 , mac_addr.a.val[3]
...
static int initialize(void)
{
	...
	{
		LOG_WRN("Broadcast source already create\r\n");
		return ret;
	}
	if (ret) {
...
int le_audio_enable(le_audio_receive_cb recv_cb)
{
 ...
 //TODO: Load all information to create advertising packet
	/* Start extended advertising LavalierMicrophone + MAC*/
