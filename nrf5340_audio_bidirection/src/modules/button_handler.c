...
static void button_scan(uint8_t button_idx)
{
	...
	if(btn_cfg[button_idx].current_status != 0)
	{
		...
		// if(btn_cfg[button_idx].button_multiple_press_count && btn_cfg[button_idx].multi_press_detect == true)
		// {
		// 	btn_cfg[button_idx].multi_press_detect = false;
	...
	{
		if(btn_cfg[button_idx].current_tick > BUTTON_PRESS && btn_cfg[button_idx].current_tick < BUTTON_LONGPRESS)
		{
			btn_cfg[button_idx].current_tick = 0;
