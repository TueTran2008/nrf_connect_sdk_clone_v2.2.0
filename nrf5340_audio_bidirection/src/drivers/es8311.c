...
int es8311_init(audio_hal_codec_config_t *codec_cfg)
{
    ...
    LOG_INF("Chip ID:0x%04x - Version: %d\r\n", id, version);
    /*
...
/*
    ...
    LOG_INF("[ES8311]: Sample Frequency:%u\n",sample_fre);
...
int es8311_config_fmt(es_i2s_fmt_t fmt)
{
    ...
    switch (fmt) {
        case AUDIO_HAL_I2S_NORMAL:
            LOG_INF("ES8311 in I2S Format");
            dac_iface &= 0xFC;
            adc_iface &= 0xFC;
            break;
        case AUDIO_HAL_I2S_LEFT:
        case AUDIO_HAL_I2S_RIGHT:
            LOG_INF("ES8311 in LJ Format");
            adc_iface &= 0xFC;
            dac_iface &= 0xFC;
            adc_iface |= 0x01;
            dac_iface |= 0x01;
        ...
        case AUDIO_HAL_I2S_DSP:
            LOG_INF("ES8311 in DSP-A Format");
            adc_iface &= 0xDC;
            dac_iface &= 0xDC;
            adc_iface |= 0x03;
            dac_iface |= 0x03;
...
int es8311_start(es_module_t mode)
{
    ...
    ret |= es8311_write_reg(ES8311_SYSTEM_REG14, 0x3A /*| 0x80*/);
    /*
    ...
    ret |= es8311_write_reg(ES8311_GPIO_REG44, 0x40 /*| 0x80*/) /*|0x80*/;
    es8311_read_all();
    return ret;
}
...
int es8311_codec_set_voice_volume(int volume)
{
    ...
    //reg = es8311_codec_get_voice_volume(&volume);
...
{
    LOG_DBG("Es8311SetVoiceMute volume:%d", enable);
    es8311_mute(enable);
    return 0;
}
